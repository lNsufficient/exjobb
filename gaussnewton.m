...
if printout == 1
    fprintf('%s \t%s \t\t%s \t%s \t\t%s \t%s \t%s \t%s \t%s\n', 'iter', 'x', 'step size', '  f(x)', 'max(abs(r))', 'norm(grad)', 'ls iter', '  lambda', 'grad`*d/norm(d)');
end
...
while norm(lambda*d) > tol && iteration < 100 % Stopping criterion
    ...
    if printout  == 1
        fprintf('%3d %8.4f \t%8.4f \t%7.4f \t%8.4f \t\t%8.4f \t%5d \t\t%8.4f \t%8.4f\n', iteration, x(1), norm(lambda*d), f(x), max(abs(r)), norm(grad_f), ls_iter, lambda, grad_f'*d/norm(d));
        for k = 2:size_x
            fprintf('\t%8.4f\n', x(k));
        end
    end
